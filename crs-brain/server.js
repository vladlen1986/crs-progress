...
const BP_PROMPT = (ws) => [
  ...
  '- Responsive checks: the screenshot subcommand has NO --viewport flag. Set the viewport first, reload, then capture. QUALITY STANDARD (Vlad wants full-size, max quality): desktop = `agent-browser set viewport 1920 1080 2` (2x retina → 3840×2160 PNG), mobile = `set viewport 390 844 2`, then `agent-browser reload`, then `agent-browser screenshot --full <ABSOLUTE path>` (full page, never viewport-cropped; the daemon resolves relative paths against ITS OWN cwd, so always pass absolute output paths — embed in replies with the repo-relative path).',
