...
(async()=>{
  ...
  await new Promise(r=>setTimeout(r,1500));
  const pre=await p.evaluate(()=>{const c=document.getElementById('uwAuto');return{exists:!!c,checked:c&&c.checked,ls:localStorage.getItem('crs-usagewin-auto'),hasOnchange:!!(c&&c.onchange)}});
  ...
  const t0=await p.evaluate(()=>({checked:document.getElementById('uwAuto').checked,ls:localStorage.getItem('crs-usagewin-auto')}));
  ...
  const t1=await p.evaluate(()=>({checked:document.getElementById('uwAuto').checked,ls:localStorage.getItem('crs-usagewin-auto')}));
