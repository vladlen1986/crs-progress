...
Capture the module's key views with `buildprint screenshot` (agent-browser under the hood): each primary view/state you audited, in **both themes** (flip the test user's `theme_is_dark`, restore after) and **both viewports** — desktop 1440×900 and mobile 390×844. NOTE: the screenshot subcommand has NO `--viewport` flag — set it first (`agent-browser set viewport 390 844`, then `reload`), capture, then set it back. Save to `crs-brain/data/screenshots/{module_id}/<view>-<dark|light>-<desktop|mobile>.png` — predictable names; these feed later UI analysis and design prototyping. READ each PNG after capture (you see images) and use what you SAW as evidence in Tasks 1/2/8; embed the most telling shots in the report as `![…](crs-brain/data/screenshots/{module_id}/….png)`. List every captured file in the report. If the CLI reports agent-browser missing, say so and skip — never fake a capture.
