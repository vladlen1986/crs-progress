...
# …edit the shredded files in the branch worktree…
buildprint check               # validation gate — must pass
buildprint apply               # compile + push to Bubble
```
...
3. **`buildprint check` must pass before `apply`. NEVER use `--force-apply`, `--no-check`, or `sync --reset --confirm`** without Vlad explicitly approving in that same conversation.
