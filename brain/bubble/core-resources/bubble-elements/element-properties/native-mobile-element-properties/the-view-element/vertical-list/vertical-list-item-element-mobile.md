# Vertical list item element (mobile)
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/native-mobile-element-properties/the-view-element/vertical-list/vertical-list-item-element-mobile · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

The vertical list item element is a child element of the vertical list element, and controls the properties of all the items in the list.
...
Swipe actions are interactive controls that appear when a user swipes left or right on a list item. When tapped, they trigger a workflow. Each swipe action can be configured with a label, an icon, or both, and can be given a background color.

Swipe actions are added as a [*SwipeAction* element](/core-resources/bubble-elements/element-properties/native-mobile-element-properties/the-view-element/vertical-list/vertical-list-item-element-mobile/swipe-action-vertical-list-element-mobile), visible in the element tree. They can only be added from the vertical list item, and do not exist in the element pallet. You will see the element in the editor on the left or right side of the vertical list item.

<table><thead><tr><th width="207.3359375">Property</th><th>Description</th></tr></thead><tbody><tr><td>Leading swipe action</td><td>An action revealed when the user swipes <strong>right</strong> on a list item, appearing on the left side. Commonly used for positive or primary actions, such as marking an item as read.</td></tr><tr><td>Trailing swipe action</td><td>An action revealed when the user swipes <strong>left</strong> on a list item, appearing on the right side. Commonly used for destructive or secondary actions, such as deleting an item.</td></tr></tbody></table>

#### Stacking swipe actions

Swipe actions can be stacked, adding multiple actions in a horizontal row — up to three per side. Each additional action adds a child element to the element tree, which can be styled and configured independently.
...
{% hint style="info" %}
**Swipe actions:** adding workflows in the Interaction tab will create an *element is tapped* workflow. If you want to use swipe actions, see the [swipe actions](#swipe-actions) section.
{% endhint %}
...
Allows the list item itself to be interactive and trigger workflows. When enabled, you can attach tap/press events directly to the list item. If you want to use swipe actions, see the [swipe actions](#swipe-actions) section.
