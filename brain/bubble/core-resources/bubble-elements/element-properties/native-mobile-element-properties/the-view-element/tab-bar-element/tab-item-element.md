# Tab item element
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/native-mobile-element-properties/the-view-element/tab-bar-element/tab-item-element · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

The tab item element is a child element of the [tab bar](/core-resources/bubble-elements/element-properties/native-mobile-element-properties/the-view-element/tab-bar-element). Items are not added from the element pallette, but with one of the two methods below:

* By adding a tab item in the [Tab bar's Tab items](/core-resources/bubble-elements/element-properties/native-mobile-element-properties/the-view-element/tab-bar-element#tab-items) section
* By enabling *Include as tab item* in a view's *Configure – Navigation* property

## Add/delete tab

Adds or deletes a tab item in the currently selected tab list.
...
Sets the view that will be loaded when the tab item is tapped.

#### Button type

Sets whether the tab item should be displayed as a label, an icon or both.

#### Icon

Sets the icon, if the [button type](#button-type) is set to *icon* or *icon and label*.

#### Text

Sets the text, if the [button type](#button-type) is set to *label* or *icon and label*.
...
Sets the styling properties of the icon, if the [button type](#button-type) is set to *icon* or *icon and label.*
