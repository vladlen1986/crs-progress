# Leading/trailing app bar button (mobile)
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/native-mobile-element-properties/the-view-element/app-bar-element-mobile/leading-trailing-app-bar-button-mobile · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

The leading button and trailing button are pseudo child elements of the App bar. They're not added from the element palette, but instead added as a property on the App bar. For each button you add, another child element is added to the element tree.

You can also add/remove sibling buttons with the *Add button* and *Delete button* buttons at the top of the property editor when a button is selected.
...
Sets the label of the button, if label or icon/label is enabled in [button type](#button-type).

#### Icon

Sets the icon of the button, if icon or icon/label is enabled in [button type](#button-type).
...
Allows the button to be interactive and trigger workflows. When enabled, you can attach tap/press events directly to the button.
