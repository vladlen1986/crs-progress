...
The section header is where you set which field the list should be grouped by. If the data type of the parent section list element is set to *User* for example, you can set the *Group by* property to a field on that user, such as their date of birth.
...
Elements in the section list header can access the header's grouping. For example, you can place a text element in the header, and use the current grouping as a data source:

{% code overflow="wrap" %}

```
Current header's Grouping's date of birth:formatted as mmmm
```
...
The field on the parent section lists's data type to group by.

#### Type of grouping

The type of grouping you want to use.

<table><thead><tr><th width="163.26171875">Group by</th><th width="172.9921875">Other types</th><th></th></tr></thead><tbody><tr><td><strong>Numbers</strong></td><td></td><td></td></tr><tr><td></td><td>Exact</td><td>Groups numbers by exact match.</td></tr><tr><td></td><td>Bucket</td><td>Groups numbers in intervals set in the <em>Configure</em> section.</td></tr><tr><td><strong>Date</strong></td><td></td><td></td></tr><tr><td></td><td>Exact</td><td>Groups dates by exact match (i.e. one group per date returned)</td></tr><tr><td></td><td>Day</td><td>Groups dates by day, in intervals set in the <em>Configure</em> section.</td></tr><tr><td></td><td>Month</td><td>Groups dates by month, in intervals set in the <em>Configure</em> section.</td></tr><tr><td><strong>Other types</strong></td><td>Exact</td><td>Other field types are grouped by exact match.</td></tr></tbody></table>
...
Lets you change the order of the element in the hierarchy.
...
#### Interval <mark style="color:$info;">(dates and numbers)</mark>

<mark style="color:$info;">If the</mark> *<mark style="color:$info;">group by</mark>* <mark style="color:$info;">propererty returns a date or number, you can set an interval that you want to group those values by. See</mark> *<mark style="color:$info;">How grouping works</mark>* <mark style="color:$info;">for more information about grouping.</mark>

#### Starting value/day/month

The grouping can have a start and end value to control the length of the list.

#### Do not skip empty groups

When checked, groups with an empty value will also be displayed.
