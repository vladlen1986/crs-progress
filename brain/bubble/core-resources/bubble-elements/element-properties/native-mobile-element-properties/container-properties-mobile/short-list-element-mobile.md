...
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/native-mobile-element-properties/container-properties-mobile/short-list-element-mobile · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

A short list is a container for displaying a fixed set of data items.

Unlike a list view, a short list loads all items at once instead of lazy-loading them as the user scrolls. This makes it best suited for small to medium-sized datasets where loading everything at once does not impact performance.

Key considerations:

* Multiple short lists can be used within the same view.
* All data is loaded immediately.
* Best used for smaller, frequently accessed data sets.

{% hint style="info" %}
Short lists contain a sub-element called ShortListItem. To learn more about the layout, style, appearance, border, background and shadow of the short list item, see this article.
{% endhint %}
...
Defines the data type that you want to display in the short list, such as *users*.
...
**If container has row layout**

| Alignment |                                                          |
| --------- | -------------------------------------------------------- |
| Top       | Element is placed at the top of the parent container.    |
| Bottom    | Element is placed at the bottom of the parent container. |

**If container has column layout**

| Alignment |                                                              |
| --------- | ------------------------------------------------------------ |
| Left      | Element is placed at the left side of the parent container.  |
| Right     | Element is placed at the right side of the parent container. |
...
Sets the minimum height of each row in the short list. Rows can expand beyond this value if their content requires more space.

#### Columns

Defines how columns are calculated within the container:

* **Fixed number**: Displays a set number of columns.
* **Fit to content**: Columns size themselves based on their content.
* **Fill container**: Columns automatically adjust to fill the available width.

#### No. of columns

Available when *Columns* is set to **Fixed number**. Specifies how many columns the short list should display.

#### Min. column width

Available when the *Column* is set to *Fit to content* or *Fill container.* Sets the minimum width of each column in the short list. Columns can expand beyond this value if their content requires more space.

#### Spacing type

Controls how space is applied between items:

* **Gap**: Adds consistent spacing between rows and columns.
* **Separators**: Displays divider lines between items.

When using **Gap**, you can define:
...
Allows the short list itself to be interactive and trigger workflows. When enabled, you can attach tap/press events directly to the short list.
