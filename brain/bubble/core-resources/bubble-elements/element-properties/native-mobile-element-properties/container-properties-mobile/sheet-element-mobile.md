# Sheet element (mobile)
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/native-mobile-element-properties/container-properties-mobile/sheet-element-mobile · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

Sheets are overlay elements that provide focused interaction while keeping the underlying view visible.

They are added directly to a view (similar to popups in the web editor) and are best used when the background context remains relevant. Sheets are opened using the *Show element* action.
...
This property defines the type of data the container holds. In most cases, elements that include a *Type of content* property also include a *Data source* property, which specifies the thing to contain (such as a specific user or other data type).

#### Data source

Defines the thing displayed in the container. The *Type of content* property determines what kind of data can be set as the data source.
...
Defines how content is displayed inside the sheet:

* **Scrollable**: Content can scroll vertically within the sheet.
* **Not scrollable**: Content is fixed and does not scroll.
* **Vertical list**: Displays content as a vertically scrolling list.
* **Section list**: Displays grouped list sections with headers.

#### Drag handle

Shows a visual handle at the top of the sheet. When enabled, users can drag the sheet to expand, collapse, or dismiss it (depending on the configured snap points).

#### Snap points

Defines the height positions the sheet can snap to when dragged.

You can set a default snap point and add additional ones. Available options include:
...
Snap points control how much of the screen the sheet occupies at different states.
...
Allows the sheet itself to be interactive and trigger workflows. When enabled, you can attach tap/press events directly to the sheet.

#### Swipe to close

Allows users to dismiss the sheet by swiping it down.

#### Block interaction behind sheet

Prevents interaction with elements behind the sheet. When enabled, users cannot tap or scroll the underlying view while the sheet is open.

#### Backdrop color

Sets the color of the overlay displayed behind the sheet. You can adjust both the color and its opacity.

#### Backdrop blur

Applies a blur effect to the content behind the sheet. Higher values increase the intensity of the blur effect.
