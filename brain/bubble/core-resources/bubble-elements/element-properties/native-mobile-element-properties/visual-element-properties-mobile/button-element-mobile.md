...
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/native-mobile-element-properties/visual-element-properties-mobile/button-element-mobile · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

Triggers a workflow when clicked.
...
A button can display a text label, an icon, or both. Use the dropdown in the *Content* section to choose the option you want.

#### Text

Sets the text displayed on the button. Only visible when *Icon and label setting* is set to *Label* or *Icon and label*.

#### Icon

Sets the icon displayed on the button. Only visible when *Icon and label setting* is set to *Icon* or *Icon and label*.
...
Sets the distance between the icon and text in pixels.

{% hint style="info" %}
This property is only visible when the *Button type* property is set to *Icon and label.*
{% endhint %}
...
{% hint style="info" %}
This section is only visible when the *Button type* property is set to *Icon* or *Icon and label.*
{% endhint %}

#### Icon size

Sets the size of the button's icon in pixels.

#### Icon color

Sets the color of the button's icon. You can choose a custom color or use a color variable.

#### Icon alignment

Determines where the icon appears relative to the label. Options are *Left*, *Top*, *Bottom*, and *Right*.
