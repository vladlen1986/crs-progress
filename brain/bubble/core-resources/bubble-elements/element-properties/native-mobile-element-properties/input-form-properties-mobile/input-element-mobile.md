...
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/native-mobile-element-properties/input-form-properties-mobile/input-element-mobile · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

Collects single-line text input from the user. Commonly used for names, emails, short responses, or numeric values.
...
Defines the hint text displayed when the input is empty.

#### Initial content

Sets a default value displayed in the input when the page loads.
...
Defines the expected format of the input. This affects validation and keyboard behavior on mobile devices.

<table><thead><tr><th width="121.16339111328125">Input format</th><th>Description</th><th>Sub-properties</th></tr></thead><tbody><tr><td>Text</td><td>Accepts any text input without format restrictions.</td><td>Max number: the maximum length of the string.</td></tr><tr><td>Email</td><td>Accepts input formatted as an email address and enables email-specific validation.</td><td></td></tr><tr><td>Password</td><td>Masks the entered text with asterisks.</td><td>Check the password while typing: if a password policy is defined in <em>Settings – General</em>, check this box to validate the input against that policy.</td></tr><tr><td>Integer</td><td>Accepts whole numbers only (no decimals).</td><td>Show thousands separator: inserts the thousands separator (<code>,</code>) as the user types. Set a range: sets the minimum and/or maximum numerical value.</td></tr><tr><td>Decimal</td><td>Accepts numeric input including decimal values. Decimal inputs use a period (<code>.</code>) as the decimal separator -- commas (<code>,</code>) aren't recognized.</td><td>Decimal place: sets the number of digits to show after the decimal point. Show thousands separator: inserts the thousands separator (<code>,</code>) as the user types. Set a range: sets the minimum and/or maximum numerical value.</td></tr><tr><td>Address</td><td>Accepts Google Maps-compatible addresses and automatically validates the address with Google Maps.</td><td></td></tr><tr><td>US phone</td><td>Accepts phone numbers formatted for US standards and may apply formatting automatically.</td><td></td></tr><tr><td>Percentage</td><td>Accepts numeric input intended to represent percentages.</td><td>Decimal place: sets the number of digits to show after the decimal point. Set a range: sets the minimum and/or maximum numerical value.</td></tr><tr><td>Currency</td><td>Accepts numeric input formatted as a currency value.</td><td>Decimal place: sets the number of digits to show after the decimal point. Set a range: sets the minimum and/or maximum numerical value.</td></tr><tr><td>Date</td><td>Accepts dates in the US date format (mm.dd.yyyy).</td><td></td></tr><tr><td>Euro date</td><td>Accepts dates in the European date format (dd.mm.yyyy).</td><td></td></tr><tr><td>Text (numbers only)</td><td>Accepts numeric characters only, but treats the value as text rather than a number.</td><td></td></tr></tbody></table>
...
Lets the user run the workflow tied to the input by pressing Enter.

#### Make required

When enabled, the user can't trigger any workflows tied to the input if its value is empty.

#### Make disabled

Disables the input field so the user can't interact with it.
...
Use `Input's value` in dynamic expressions or workflows.

**How do I reset an input?**

Use the *Reset relevant inputs* action, or reset the parent container with *Reset a group/popup*. Keep in mind that resetting an input returns it to its original initial content -- it doesn't clear the value entirely.
