# Selectable list element (mobile)
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/native-mobile-element-properties/input-form-properties-mobile/selectable-list-element-mobile · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

A selectable list lets users select one or multiple options from a list. It's commonly used for filters, segmented controls, and selection forms without requiring custom states or additional workflows. The element supports both single-select and multi-select modes, with options to control selection behavior and connect selected values to workflows.
...
Defines how many items users can select. *Single select* allows selection of one item. *Multi select* allows selection of multiple items.

#### Choices

Specifies the list of selectable items. Can be set as a static list or generated dynamically from a data source.

#### Default value

Sets the initially selected item or items. Must match the data type of the defined choices.
...
Sets the minimum height of each item row. Rows expand automatically if their content requires more space.

#### Columns

Defines how columns are arranged. *Fixed number* displays a set number of columns. *Fit to content* sizes columns based on their content. *Fill container* expands columns to fill the available width.

#### No. of columns

Available when *Fixed number* is selected. Specifies how many columns to display.

#### Spacing type

Controls spacing between items. *Gap* adds configurable spacing between rows and columns. *Separators* displays divider lines between items.
...
Allows the selectable list to trigger workflows. When enabled, you can attach tap/press events directly to the element.

#### Make required

When enabled, the user can't trigger any workflows tied to the selectable list if no item is selected.

#### Make disabled

Disables the selectable list so the user can't interact with it.
