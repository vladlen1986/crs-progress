# Checkbox element (mobile)
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/native-mobile-element-properties/input-form-properties-mobile/checkbox-element-mobile · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)
...
Defines how the checkbox is displayed. *Icon and Label* shows the checkbox icon along with a text label. *Icon only* displays only the checkbox icon without a label.

#### Label

Sets the text displayed next to the checkbox when *Icon and Label* is selected.

#### Default state

Defines whether the checkbox is checked or unchecked when the view loads. Supports a dynamic value.
...
Selects the icon used for the checkbox.

#### Icon size

Sets the size of the checkbox icon in pixels.

#### Icon color

Defines the color of the checkbox icon. You can adjust both the color value and its opacity, and style variables are supported.
...
When enabled, the user can't trigger any workflows tied to the checkbox if it isn't checked.
