# Multiline input (mobile)
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/native-mobile-element-properties/input-form-properties-mobile/multiline-input-mobile · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

Collects multi-line text input from the user. Typically used for longer responses such as descriptions or messages. The multiline input supports plaintext[^1] only -- it's not a rich text editor.
...
Defines the hint text displayed when the multiline input is empty.

#### Initial content

Sets a default value displayed in the multiline input when the page loads.
...
Enable this to make the element [visible by default](#user-content-fn-3)[^3]. This checkbox makes the element visible every time the view loads. Change the visibility of the element based on certain conditions[^4] in the Conditional section in the property editor or with a show/hide element action[^5] in a workflow.
...
When enabled, the user can't trigger any workflows tied to the multiline input if its value is empty.

#### Make disabled

Disables the multiline input field so the user can't interact with it.
...
**When should I use a multiline input?**

Use it when you expect longer or paragraph-style responses without rich text formatting, such as chat messages, product descriptions, or user bios.

**Does the multiline input support rich text formatting?**

No, the multiline input supports plaintext[^1] only.

**How do I reset a multiline input?**

Use the *Reset relevant inputs* action, or reset the parent container with *Reset a group/popup*. Keep in mind that resetting a multiline input returns it to its original initial content -- it doesn't clear the value entirely.

</details>

[^1]: *Plaintext* is text that contains no formatting. It doesn't include styling such as bold, italics, font sizes, colors, links, or embedded elements. It's stored and displayed exactly as written, as simple characters only.
