# Searchbox element
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/web-element-properties/input-form-properties/searchbox-element · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

Lets users search and select from a dynamic list of options by typing into an input. As they type, matching options appear in a dropdown for them to choose from.
...
Defines the hint text displayed when the searchbox is empty.

#### Choices

Defines the list of options available in the searchbox. Set as *Static* to provide a fixed list of options separated by a line break, or *Dynamic* to generate the list using a dynamic expression.

#### Default value

Sets the default value of the searchbox.
...
The maximum number of items to show in the search result.
...
Lets the user run the workflow tied to the searchbox by pressing Enter.

#### Allow entries not in list

Lets users enter a value that isn't included in the list. The custom entry can be accessed in a dynamic expression using `Searchbox's typed text`.

#### Make required

When enabled, the user can't trigger any workflows tied to the searchbox if its value is empty.

#### Make disabled

Disables the searchbox so the user can't interact with it.
...
**What's the difference between a dropdown and a searchbox?**

A dropdown shows all options at once. A searchbox dynamically filters results as the user types. A searchbox also allows entries that aren't in the list, if you enable *Allow entries not in list* in the *Interaction* section.

**How do I access the text the user typed, even if it doesn't match an item in the list?**

Enable *Allow entries not in list* in the *Interaction* section, then reference `Searchbox's typed text` in a dynamic expression.

**How do I access the selected result?**

Use `Searchbox's value`.
