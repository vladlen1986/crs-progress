# Dropdown element
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/web-element-properties/input-form-properties/dropdown-element · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)
...
Defines the hint text displayed when no option has been selected.
...
Defines the list of options available in the dropdown. Set as *Static* to provide a fixed list of options separated by a line break, or *Dynamic* to generate the list using a dynamic expression.

#### Default value

Sets the default selected value of the dropdown.
...
When enabled, the user can't trigger any workflows tied to the dropdown if its value is empty.

#### Make disabled

Disables the dropdown so the user can't interact with it.
...
Use `Dropdown's value`.

**Can I use dynamic data?**

Yes. Set the choices source to a dynamic expression that returns a list.

**When should I use the dropdown element?**

Use a dropdown when you want users to select a single value from a controlled list of options, such as a list of countries, user roles, or categories.
