...
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/web-element-properties/input-form-properties/radio-buttons-element · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

Allows users to select one option from a visible list of choices.
...
Defines how the list of options is provided. Set as *Static* to enter a fixed list of text options each separated by a line break, or *Dynamic* to generate the list using a dynamic expression.
...
Sets the number of columns the radio buttons are divided into. Can be a static or dynamic value. Choosing *Dynamic* reveals an additional property:

* **Min column width:** sets the smallest width each column can have. This lets Bubble automatically adjust the number of columns based on available space, while ensuring no column becomes narrower than the defined minimum.

#### Bootstrap color

Sets the predefined Bootstrap[^1] style applied to the selected radio button. Choose from the standard contextual color options (such as primary, success, warning, or danger) to control how the selected state appears. This affects visual styling only and doesn't change the element's behavior.
...
When enabled, the user can't trigger any workflows tied to the radio button element if its value is empty.

#### Make disabled

Disables the radio button element so the user can't interact with it.
...
Use radio buttons when you want all options visible at once. Radio buttons let users select an option with a single click, while dropdowns require an extra step to open the list.

**How do I access the selected value?**

Use `Radio buttons's value`.

</details>

[^1]: *Bootstrap* is an open-source front-end framework used to design and style web applications.
