...
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/web-element-properties/input-form-properties/file-uploader-element · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)
...
Defines the file shown before a value is entered into the element.
...
Sets the default value as a static uploaded file or a dynamic expression that returns a file URL.
...
Sets the maximum allowed file size in megabytes. You can define this as a fixed value or generate it using a dynamic expression.
...
When enabled, the user can't trigger any workflows tied to the file uploader if its value is empty.

#### Make disabled

Disables the file uploader so the user can't interact with it.
...
Use `File uploader's value`, which returns the file.

**Where is the file stored?**

Uploaded files are stored in Bubble's file storage, unless you pick a different storage service in *Visual - Configure - Storage service.*
