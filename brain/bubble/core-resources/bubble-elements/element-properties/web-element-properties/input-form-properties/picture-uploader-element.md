...
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/web-element-properties/input-form-properties/picture-uploader-element · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)
...
Defines the image displayed before a file is uploaded.
...
Sets the default image displayed in the uploader.
...
By default, uploaded files are stored by Bubble using Amazon S3. You can optionally use a third-party storage service by installing the corresponding plugin. Currently, Bubble supports one third-party service: Box.

{% hint style="info" %}
You need to install the [Box plugin](https://manual.bubble.io/core-resources/bubble-made-plugins/box) to upload files to Box. The option won't appear until the plugin is installed.
{% endhint %}

#### Make private

By default, uploaded files are publicly accessible to anyone with the file's URL. Enable this option to attach the file to a specific thing in your database, then control access using privacy rules for that data type.

{% hint style="warning" %}
If you're unfamiliar with using privacy rules to protect uploaded files, read [Uploading private files](https://manual.bubble.io/help-guides/data/files#uploading-private-files) before enabling this option.
{% endhint %}

Enabling this option reveals an additional property:

* **Attach this file to:** defines the data type the securely stored file will be attached to.

#### Resize images over 800px x 600px

When enabled, images larger than 800 x 600 pixels are automatically resized to those dimensions before uploading. Images at or below that size are uploaded at their original dimensions.
...
When enabled, the user can't trigger any workflows tied to the picture uploader if its value is empty.

#### Make disabled

Disables the picture uploader so the user can't interact with it.
...
**How do I access the uploaded image?**

Use `Picture uploader's value`, which returns the image file.

**Where is the image stored?**

Uploaded images are stored in Bubble's file storage, unless you pick a different storage service in *Visual - Configure - Storage service.*

**Are uploaded images public or private?**

By default, uploaded files are public; anyone with the file's URL can access them. You can protect uploaded files using privacy rules, which restrict access based on the current user's permissions for the associated data.
