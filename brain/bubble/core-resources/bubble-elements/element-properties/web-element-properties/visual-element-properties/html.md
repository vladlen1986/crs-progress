# HTML
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/web-element-properties/visual-element-properties/html · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

This element renders HTML directly on the page. Some services provide embeddable HTML snippets that can be placed here to render third-party content or components.
...
The HTML to render. Script tags only execute in run mode, not in the editor. Use the *Insert dynamic data* button to include dynamic values in the HTML.
...
When checked, the HTML runs in an iframe, as if it were an independent web page. Check the documentation of the service providing the code to determine whether this is a relevant option.
...
When enabled, the HTML element delays rendering until it becomes visible on the screen. This can improve page load performance by avoiding unnecessary rendering of off-screen content.
