# Video element
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/web-element-properties/visual-element-properties/video-element · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

This element displays a video player on the page, supporting YouTube and Vimeo. You can also add Ziggeo as a supported source by installing the plugin.

{% hint style="info" %}
The Bubble engine can't detect whether an embedded video is playing or paused. A video that has started won't stop on its own, even if its container is hidden. To stop a video when its container is hidden, use a conditional to set the video source to *null* when the container's visibility changes.
{% endhint %}
...
The hosting service, such as Youtube.

#### Video ID

The ID of the video to display. This is typically the ID found in the video's URL on YouTube or Vimeo.

{% hint style="info" %}
If your app uses opt-in cookies, users must consent before a YouTube video will load.
{% endhint %}
...
When enabled, the video starts playing automatically when the page loads.

#### Loop on repeat

When enabled, the video loops continuously after it finishes playing.
