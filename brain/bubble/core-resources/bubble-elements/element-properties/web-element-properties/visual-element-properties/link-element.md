# Link element
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/web-element-properties/visual-element-properties/link-element · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

Navigates to another page or URL when clicked. Unlike buttons, links render as `<a>` tags, which web crawlers such as search engines can recognize and index.
...
Replaces the standard text link with an icon. When enabled, you can configure the icon using the same settings available for the Icon element.

#### Text

Sets the text displayed for the link. This is separate from the URL the link points to, meaning the visible label can differ from the destination URL.
...
Sets where the link navigates to. *Internal page* links to a page within your app and remains valid even if the page is renamed. *External URL* links to a specific URL, which can be static or generated using a dynamic expression.

#### Destination page

The page to navigate to. This property is only visible if [*Link destination*](#link-destination) is set to *Internal page.*

#### **Parameters**

Lets you add URL parameters to the URL.
...
When enabled, the link opens in a new browser tab and sets focus to that tab.

Makes the element clickable, allowing you to attach click events and trigger workflows.

#### Mark the link as nofollow

Adds a `rel="nofollow"` attribute to the link, signalling to search engines that they shouldn't pass ranking authority to the linked page.
