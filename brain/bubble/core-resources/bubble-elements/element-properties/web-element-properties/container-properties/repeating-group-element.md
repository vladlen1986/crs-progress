...
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/web-element-properties/container-properties/repeating-group-element · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

{% hint style="info" %}
For a more in-depth look at repeating groups, read our dedicated User Manual article on the subject:

Article: [Repeating groups](/help-guides/design/elements/web-app/containers/repeating-groups)
{% endhint %}

A repeating group is a container that displays a list of items by repeating its cell layout once per item in a data source. Each cell represents a single item from that list.

Like a group, a repeating group defines a data context for its child elements. However, instead of one shared data source for the entire container, each cell has its own data context. If the repeating group’s data source is `Do a search for:Users`, each cell represents one User from the search results.

Child elements inside a cell can reference that item using `Current cell's` expressions, such as `Current cell's User's email`.
...
Defines the data type that you want to display in the repeating group, such as *users*.
...
Determines how the number of rows is calculated.

* **Fixed number**: Displays a specific number of rows, defined in *# of rows*.
* **Fit to content**: Automatically adjusts the number of rows based on the number of items in the data source.
* **Fill container**: Adjusts the number of rows to fill the available vertical space in the container.

#### # of rows

Specifies the exact number of rows when *Rows* is set to *Fixed number*.

#### Columns

Determines how the number of columns is calculated.

* **Fixed number**: Displays a specific number of columns, defined in *# of columns*.
* **Fit to content**: Automatically adjusts the number of columns based on the number of items in the data source.
* **Fill container**: Adjusts the number of columns to fill the available horizontal space in the container.

#### # of columns

Specifies the exact number of columns when *Columns* is set to *Fixed number*.

#### Separators

Defines the visual divider between cells. You can select the style (for example, solid) and the thickness in pixels. Separators help visually distinguish rows and columns within the repeating group.
...
Enable this option to allow the last page of a paginated table to display fewer than the specified number of rows or columns, if there aren’t enough items to fill the full page.
...
<summary>Repeating group element: <strong>FAQ</strong></summary>

### When should I use a repeating group instead of a group?

Use a repeating group when you need to display a list of items from the database or another list-based data source. Each cell represents one item in the list and has its own data context.

Use a group when you only need to display a single item or organize static elements without iterating over a list.

### How does the repeating group’s data source work?

The repeating group’s *data source* must return a list. For example, `Do a search for:Users` returns a list of users. Each item in that list becomes one cell.

If the data source returns an empty list, no cells are displayed.

### Can I place a repeating group inside another repeating group?

Yes. Nested repeating groups are supported.

Be mindful of performance when nesting large datasets. Each nested repeating group runs its own search. For example, if a parent repeating group displays 25 rows and each row contains a nested repeating group with its own search, the page performs:

* 1 search for the parent repeating group
* 25 additional searches (one per row for the nested repeating group)

This results in 26 searches in total. This pattern can significantly slow down page load times and increase workload usage.

### Can I sort data in a repeating group?

Yes. Sorting can be configured directly in the data source. For example, when using `Do a search for`, you can set the *Sorting* property to a specific field (such as a user’s last name) and choose ascending or descending order.

You can also use the `:sorted` operator on any list. This allows you to sort lists that are not generated by a search, such as option sets.
...
[^3]: The exception is is if you use the Go to page action and go to the same page you are currently on – in this case the element will keep its current visibility, since the page is not refreshed.
    ...
    Reference: [Action: Go to page](https://manual.bubble.io/core-resources/bubble-elements/element-properties/web-element-properties/container-properties/pages/-MTujrgFLiHe7MfwSBOF#go-to-page-...)<br>
