# Table element
> Source: https://manual.bubble.io/core-resources/bubble-elements/element-properties/web-element-properties/container-properties/table-element · Captured: 2026-07-14 (verbatim from manual.bubble.io llms-full.txt)

A Table is a structured container that displays tabular data in predefined rows and columns. Each row represents a single item from a data source, and each column represents a specific field or expression. Unlike a repeating group, a table enforces a column-based structure and is optimized for displaying consistent, grid-style datasets rather than fully custom cell layouts.

{% hint style="info" %}
The table property is divided into child-elements, and each child element has its own set of properties. For example, with its default properties, a table consists of:

* **Table:** the table itself, which contains the child elements.
  * [**Row/column**](/core-resources/bubble-elements/element-properties/web-element-properties/container-properties/table-element/table-row-column)**:** the properties of the first row or column in the table (frequently used for headers)
    * [**Cell**](/core-resources/bubble-elements/element-properties/web-element-properties/container-properties/table-element/table-row-column/table-row-column-cell)**:** the properties of cells in the row or column.
  * [**Row/column repeated:**](/core-resources/bubble-elements/element-properties/web-element-properties/container-properties/table-element/table-row-column-repeating) the properties of repeated rows/columns in the table (the table content)
    * [**Cell**](/core-resources/bubble-elements/element-properties/web-element-properties/container-properties/table-element/table-row-column/table-row-column-cell)**:** the properties of cells in the row or column.

The properties in this article describe the properties on the parent **table** element.
{% endhint %}
...
Defines the data type that you want to display in the table, such as *users*.
...
Determines how rows and columns are organized within the table.

* **Vertical** – Displays items as rows, with columns representing fields or expressions. This is the standard table layout.
* **Horizontal** – Displays items as columns, with rows representing fields or expressions.

#### Separators

Controls the visual dividers between rows and columns. You can enable and configure vertical and horizontal separators independently.

* **Vertical:** Sets the style and thickness of the vertical lines between columns.
* **Horizontal:** Sets the style and thickness of the horizontal lines between rows.
...
<summary>Table element: <strong>FAQ</strong></summary>

#### Can I sort data in a table?

es. Sorting can be configured directly in the data source. For example, when using `Do a search for`, you can set the *Sorting* field to a specific field (such as a user’s last name) and choose ascending or descending order.
...
Technically, you can place a table inside another container that is itself part of a list structure. However, nesting large datasets can significantly increase the number of searches performed and impact performance. Optimize your data model and limit nested searches where possible.

#### Does a table improve performance compared to a repeating group?

A table provides a more structured layout and can be easier to manage for tabular data. However, performance primarily depends on the size of the dataset and the complexity of expressions used. Limiting the number of rows and using pagination helps maintain performance in both tables and repeating groups.
