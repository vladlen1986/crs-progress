...
- App-level (settings/client-safe.json): `exposes_wf_api: true`, `exposes_get_api: true` (Data GET API on app-wide), `allow_iframe: "allow"` (clickjacking surface), `twofa_auth: false`, `have_pw_policy: false`, SendGrid enabled but `sendgrid_verified: false`, Google Maps key client-exposed (restricted). External script loaded from crs.statuspage.io in header. **Re-verified 2026-07-16 against snapshot `be1670b3`** (the only file that sync touched was client-safe.json): every flag above unchanged; also confirmed `app_rights: "private"`, `pw_protection: false` (test app not password-protected), `remove_bubble_console: true`.
